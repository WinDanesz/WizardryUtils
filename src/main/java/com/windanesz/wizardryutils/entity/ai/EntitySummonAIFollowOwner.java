...
/**
 * AI class based on {@link EntityAIBase}, which makes summons follow their owners. Should work both with {@link ISummonedCreature} and {@link SummonedCreatureData}.
 */
public class EntitySummonAIFollowOwner extends EntityAIBase {
	...
	private boolean dynamicSummon;

	// fields for "caching"
	private SummonedCreatureData capability;
	private ISummonedCreature iSummonedCreature;
	...
	public EntitySummonAIFollowOwner(EntityCreature summonedCreature, double followSpeedIn, float minDistIn, float maxDistIn) {
		...
		if (!(summonedCreature instanceof ISummonedCreature || SummonedCreatureData.isSummonedEntity(summonedCreature))) {
			throw new IllegalArgumentException("Entities with EntitySummonAIFollowOwner must implement ISummonedCreature or be summoned through the SummonedCreatureData capability!");
		}

		if (summonedCreature instanceof ISummonedCreature) {
			iSummonedCreature = (ISummonedCreature) summonedCreature;
		} else {
			dynamicSummon = true;
			capability = SummonedCreatureData.get(summonedCreature);
		}
	...
	public boolean shouldExecute() {
		EntityLivingBase ownerCaster = dynamicSummon ? capability.getCaster() : iSummonedCreature.getCaster();
