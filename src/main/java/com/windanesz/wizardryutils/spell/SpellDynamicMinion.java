...
/**
 * Based on {@link SpellMinion} - Author: Electroblob
 * This is an altered version of {@link SpellMinion}, using capabilities ({@link SummonedCreatureData}) to achieve minion spells
 * without re-registering entities as separate minion entities, allowing a much more portable execution.
 *
...
public class SpellDynamicMinion<T extends EntityCreature> extends Spell {
	...
	public boolean cast(World world, EntityPlayer caster, EnumHand hand, int ticksInUse, SpellModifiers modifiers) {
		if (!extraConditions(new BlockPos(caster.posX, caster.posY, caster.posZ), caster, modifiers, world)) {
			return false;
		}
	...
	public boolean cast(World world, double x, double y, double z, EnumFacing direction, int ticksInUse, int duration, SpellModifiers modifiers) {
		if (!extraConditions(new BlockPos(x, y, z), null, modifiers, world)) {
			return false;
		}
	 ...
	 * {@link SpellDynamicMinion#MINION_COUNT} property within a number of blocks of the caster specified by the property
	 * {@link SpellDynamicMinion#SUMMON_RADIUS}, returning false if there is no space to spawn the minions. Override to do
	 ...
	 * @see SpellDynamicMinion#addMinionExtras(EntityCreature, BlockPos, EntityLivingBase, SpellModifiers, int)
	 */
	 ...
	 * method in {@link SpellDynamicMinion#minionFactory}. Override to add logic for changing the type of entity summoned.
	 *
	 ...
	 *                       {@link SpellDynamicMinion#MINION_COUNT}.
	 */
	...
	/**
	 * Called just before each cast method on both sides, allowing to add some custom checks, does nothing by default.
	 */
